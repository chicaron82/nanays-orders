...
interface Props {
  stock: Stock;
  orders: Order[];
  updateStock: (updates: Partial<Stock>, opts?: { silent?: boolean }) => Promise<void>;
}

type NumericStockKey = 'lumpia_sets' | 'wrapper_packs' | 'pancit_full' | 'pancit_half' | 'pancit_large' | 'pork_frozen' | 'pork_thawed' | 'noodle_packs';
type StockStatusKey = 'carrots_status' | 'celery_status';
type UpdateFn = (s: Stock) => Stock;
...
const statusItems: { key: StockStatusKey; label: string }[] = [
...
const porkItems: { key: NumericStockKey; label: string }[] = [
...
export default function StockManager({ stock, orders, updateStock }: Props) {
  // Track the last stock prop we've seen so we can reset local edit state when
  // a Supabase realtime push changes it — without using a useEffect (which would
  // cause a cascading render). React re-renders immediately on a setState-during-render
  // and skips painting the intermediate state. See: react.dev/learn/you-might-not-need-an-effect
  const [stockSnapshot, setStockSnapshot] = useState(stock);
  const [stockEdit, setStockEdit] = useState<Stock>({ ...stock });
  if (stockSnapshot !== stock) {
  ...
  const handleUpdate = () => { updateStock(stockEdit); };
  ...
  const numVal = (key: NumericStockKey) => stockEdit[key] ?? 0;
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 pb-12 space-y-4">
      ...
      {displayItems.map(({ key, label, avail: a, reserved: r, total: t, isWrapper }) => {
        const level: 'danger' | 'warn' | 'ok' = a <= 0 ? 'danger' : a <= 2 ? 'warn' : 'ok';
        const pct = t > 0 ? Math.min(100, Math.max(0, (a / t) * 100)) : 0;
        return (
          <div key={key} className="bg-white/95 backdrop-blur-sm rounded-2xl p-5 shadow-sm border border-stone-200">
            ...
            <div className="mt-2 text-xs font-bold">
              {isWrapper && (stock.wrapper_packs ?? 0) > 0 && <span className="text-emerald-600">✅ Can make {stock.wrapper_packs} more batch{stock.wrapper_packs !== 1 ? 'es' : ''}</span>}
              {isWrapper && !stock.wrapper_packs && <span className="text-red-600 flex items-center gap-1"><AlertTriangle size={14}/> Need to buy wrappers!</span>}
      ...
      <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-6 shadow-md border border-stone-200">
          ...
          {statusItems.map(({ key, label }) => (
            <div key={key} className="flex items-center justify-between mb-3">
              ...
              <div className="flex rounded-lg overflow-hidden border border-stone-200 text-xs font-bold">
                {statusOptions.map(({ value, label: optLabel, active, inactive }) => (
                  <button
                    key={value}
                    onClick={() => setStockEdit(s => ({ ...s, [key]: value } as Stock))}
        ...
        <div className="mb-6">
          ...
          {porkItems.map(({ key, label }) => (
            <div key={key} className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-stone-700">{label}</span>
              <div className="flex items-center gap-2">
                <button className="w-8 h-8 rounded-lg border-2 border-orange-200 text-orange-600 font-bold hover:bg-orange-50 transition-colors" onClick={() => decNum(key)}>−</button>
                <span className="text-base font-bold w-6 text-center text-stone-800">{numVal(key)}</span>
                <button className="w-8 h-8 rounded-lg border-2 border-orange-200 text-orange-600 font-bold hover:bg-orange-50 transition-colors" onClick={() => incNum(key)}>+</button>
              </div>
            </div>
          ))}
          {(() => {
        ...
        <div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-base font-bold w-6 text-center text-stone-800">{numVal('noodle_packs')}</span>
              <button className="w-8 h-8 rounded-lg border-2 border-orange-200 text-orange-600 font-bold hover:bg-orange-50 transition-colors" onClick={() => incNum('noodle_packs')}>+</button>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-6 shadow-md border border-stone-200 mt-6">
        ...
        <div className="space-y-4">
          {updateItems.map(({ key, label, onInc, onDec }) => (
            <div key={key}>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-lg font-bold w-8 text-center text-stone-800">{numVal(key)}</span>
                <button className="w-10 h-10 rounded-lg border-2 border-orange-200 text-orange-600 font-bold text-xl hover:bg-orange-50 transition-colors"
                  onClick={() => setStockEdit(onInc ?? (s => ({ ...s, [key]: (s[key] ?? 0) + 1 })))}>+</button>
                <input type="number" min={0} value={numVal(key)}
